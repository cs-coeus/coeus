...
class ModelSummarizer(ModelInterface):
    ...
    def __init__(self, model_name='t5-base', framework='tf'):
        ...
        ModelSummarizer.summarizer = pipeline("summarization", model=model_name, tokenizer=model_name, framework=framework)
    ...
    def summarize_with_pipeline(text, **kwargs):
        ...
        if length < 14:
            ...
            return ""
        output = ModelSummarizer.summarizer(text, max_length=int(length * 1 / 2), min_length=int(length * 1 / 10), **kwargs)
